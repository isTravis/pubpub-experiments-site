	...
	render() {
		...
		return (
			<div>
				...
				<DataTable
					groupATitle={'Not Interested'}
					groupBTitle={'Interested'}
					utils={utils}
					groupABeefTotal={beefStats.nonInterestedAll.length}
					groupABeefError={beefStats.nonInterestedFoundError.length}
					groupABeefConclusion={beefStats.nonInterestedFoundConclusion.length}
					groupBBeefTotal={beefStats.interestedAll.length}
					groupBBeefError={beefStats.interestedFoundError.length}
					groupBBeefConclusion={beefStats.interestedFoundConclusion.length}

					groupADinoTotal={dinoStats.nonInterestedAll.length}
					groupADinoError={dinoStats.nonInterestedFoundError.length}
					groupADinoConclusion={dinoStats.nonInterestedFoundConclusion.length}
					groupBDinoTotal={dinoStats.interestedAll.length}
					groupBDinoError={dinoStats.interestedFoundError.length}
					groupBDinoConclusion={dinoStats.interestedFoundConclusion.length}

					groupAGovtTotal={govtStats.nonInterestedAll.length}
					groupAGovtError={govtStats.nonInterestedFoundError.length}
					groupAGovtConclusion={govtStats.nonInterestedFoundConclusion.length}
					groupBGovtTotal={govtStats.interestedAll.length}
					groupBGovtError={govtStats.interestedFoundError.length}
					groupBGovtConclusion={govtStats.interestedFoundConclusion.length} />
			</div>
		);
	}
});
